const REV = 6,
       BRUSHES = [["web", "Web"],
	    		["vmirrorweb","Web (Vertical Mirror)"],
	    		["hmirrorweb", "Web (Horizontal Mirror)"],
	    		["",""],
	    		["sketchy","Sketchy"],
	    		["vmirrorsketchy","Sketchy (Vertical Mirror)"],
	    		["hmirrorsketchy","Sketchy (Horizontal Mirror)"],
	    		["",""],
	    		["shaded","Shaded"],
	    		["vmirrorshaded","Shaded (Vertical Mirror)"],
	    		["hmirrorshaded","Shaded (Horizontal Mirror)"],
	    		["",""],
	    		["simple","Simple"],
	    		["vmirrorsimple","Simple (Vertical Mirror)"],
	    		["hmirrorsimple","Simple (Horizontal Mirror)"],
	    		["",""],
	    		["chrome","Chrome"],
	    		["fur","Furry"],
	    		["longfur","Longer Furry"],
	    		["",""],
	    		["squares","Square Trail"],
	    		["ribbon","Ribbon Trail"],
	    		["",""],
	    		["circles","Grid of Circles"],
	    		["grid",""]],
...
//	menu.about.addEventListener('touchend', onMenuAbout, false);
	...
	if (window.location.hash)
	{
		...
		for (i = 0; i < BRUSHES.length; i++)
		{
			if (hash == BRUSHES[i][0])
			{
				brush = eval("new " + BRUSHES[i][0] + "(context)");
	...
	if (!brush)
	{
		brush = eval("new " + BRUSHES[0][0] + "(context)");
	}
...
function onWindowKeyUp( event )
{
	switch(event.keyCode)
	{
		...
		case 82: // r
			brush.destroy();
			brush = eval("new " + BRUSHES[menu.selector.selectedIndex][0] + "(context)");
			break;
		case 66: // b
...
{
	if (BRUSHES[menu.selector.selectedIndex][0] == "")
		return;

	brush.destroy();
	brush = eval("new " + BRUSHES[menu.selector.selectedIndex][0] + "(context)");

	window.location.hash = BRUSHES[menu.selector.selectedIndex][0];
}
